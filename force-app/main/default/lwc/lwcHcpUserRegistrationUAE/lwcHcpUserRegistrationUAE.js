...
import HCP_COUNTRY from '@salesforce/label/c.HCPCountry';
...
export default class lwcHcpUserRegistrationUAE extends LightningElement {
    //urls
    ...
    confirmEmailFocus(event) {
       const style = document.createElement('style');
        style.innerText = `c-lwc-hcp-user-registration-u-a-e .email-address .slds-input {
            color: transparent;
    ...
    connectedCallback() {
        ...
        .then(data => {
            ...
            this.isMedicalIdVisible = this.isDoctor;
        })
        .catch(error => {
            console.log(error);
            this.showContent = true;
    ...
    validateEmailAddress(event) {
        ...
        validateEmailAddressFunc(inputCmp, 'United Arab Emirates', this.hcpEmailExists, this.hcpEmailInvalid, this.template, this.tabEventInProgress)
        .then(result => {
    ...
    //set medicalId_pattern based on country selected
    handleMedicalIdPattern(event) {
        const selectedCountry = event.target.value ? event.target.value  : '';
					switch (selectedCountry) {
						case 'Bahrain':
							this.medicalId_pattern = '^[0-9]{8}$';
							break;
						case 'Egypt':
							this.medicalId_pattern = '^[0-9]{6}$';
							break;
						case 'Iraq':
							this.medicalId_pattern = '^[0-9]{5}$';
							break;
						case 'Jordan':
							this.medicalId_pattern = '^[0-9]{5}$';
							break;
						case 'Kuwait':
							this.medicalId_pattern = '^[0-9]{9}$';
							break;
						case 'Lebanon':
							this.medicalId_pattern = '^[0-9]{4}\/[0-9]{1}\/[0-9]{4}$';
							break;
						case 'Oman':
							this.medicalId_pattern = '^[0-9]{5}$';
							break;
						case 'Qatar':
							this.medicalId_pattern = '^[0-9]{11}$';
							break;
						case 'Saudi Arabia':
							this.medicalId_pattern = '^[0-9]{10}$';
							break;
						default:
							this.medicalId_pattern = '^[0-9]{8}\-[0-9]{3}$';
							break;
					}
    ...
    handleProfessionChange(event) {
        const selectedProfession = event.target.value ? event.target.value  : '';
        this.isMedicalIdVisible = !(selectedProfession == 'Nurse' || selectedProfession == 'Clinic Manager')
    }
